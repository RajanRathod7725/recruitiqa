...
class Recover_password extends CI_Controller {
    public function __construct()
    {
        ...
        $this->thisModuleBaseUrl = site_url('employer/recover_password').'/';
    ...
    public function index($Token)
    {
        ...
        $data['action'] = site_url('employer/recover_password/insert/' . $Token);
        ...
        $value=('employer.status,employer.employer_id,employer_forget_password.created_at');
        $joins = array
        (
            array
            (
                'table' => 'employer',
                'condition' => 'employer_forget_password.employer_id = employer.employer_id',
                'jointype' => 'left'
            ),
        );
        $result = $this->database_model->get_joins('employer_forget_password',$value,$joins,array('employer_forget_password.employer_token'=>$Token),'employer.employer_id','asc',1)->row();
        if(empty($result))
        {
        ...
        else
        {
            ...
            if($difference > 5)
            {
                $this->database_model->delete('employer_forget_password',array('employer_id'=>$result->employer_id));
                $this->session->set_flashdata('error',$this->lang->line('gen_link_expierd'));
                redirect('employer/forgot');
                die();
            }
            else
            {
                ...
                $this->load->view('employer/recover_password', $data);
            }
    ...
    function insert($Token)
    {
        ...
        if($this->form_validation->run() === TRUE && $csrf_check==true)
        {
            ...
            else
            {
                ...
                if($difference > 5)
                {
                    ...
                    $this->database_model->update($data['tbl'],$value_array,array('employer_id'=>$result->employer_id));
                    $this->database_model->delete('employer_forget_password',array('employer_id'=>$result->employer_id));
        ...
        else
        {
            ...
            redirect(site_url('employer/recover_password/index/'.$Token));
        }
    }
    //SET FORM DATA
