...
class Ajax_move_task extends CI_Controller {
    ...
    public function index()
    {
        ...
        if($csrf_check==true)
        {
            ...
            $update_array= array(
                'todo_status_id'=>$PostArray['column'],
                'updated_at'=>$this->ip_date->cur_date,
            );
            $this->database_model->update('todo',$update_array,array('todo_id'=>$PostArray['task']));

            $value_array = array(
                'table_id' => $PostArray['task'],
                'table_name' => 'tbl_todo',
                'activity' => '2',
