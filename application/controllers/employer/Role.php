...
class Role Extends CI_Controller {
    ...
    public function __construct()
	{
        ...
        $this->thisModuleBaseUrl = site_url('employer/role').'/';
    ...
    function index($offset=0)
    {
        ...
        //$where_array=array('role.status !='=>'3','role.role_for'=>2,'role.employer_id'=>$this->session->userdata('employer_id'));
        $where_array='tbl_role.status !="3" AND (tbl_role.role_for=2 OR tbl_role.employer_id='.$this->session->userdata('employer_id').")";
        ...
        $this->load->view('employer/role', $data);
    }
    ...
    function add()
    {
        ...
        *******************************/
        $data['unique_form_name'] = "CSRFGuard_".mt_rand(0,mt_getrandmax());
        $data['token'] = $this->common_model->csrfguard_generate_token($data['unique_form_name']);
        $this->load->view('employer/role_edit', $data);
    }
    ...
    function permission_add($id='')
    {
        ...
        if(!empty($result))
        {
            ...
            $data['unique_form_name'] = "CSRFGuard_".mt_rand(0,mt_getrandmax());
            $data['token'] = $this->common_model->csrfguard_generate_token($data['unique_form_name']);
            $this->load->view('employer/permission_edit', $data);
        }
        else
        {
            $this->add();
