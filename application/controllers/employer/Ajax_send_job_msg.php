...
class Ajax_send_job_msg extends CI_Controller {
    ...
    public function index()
    {
        ...
        if($csrf_check==true)
        {
            ...
            'user_id'=>$this->session->userdata('employer_id'),
            'user_type'=>1,
            ...
            //html
            $html = '<div class="chat"> <div class="chat-avatar"> <a class="avatar m-0" data-toggle="tooltip" href="javascript:;" data-placement="bottom" title="'.$this->session->userdata('employer_name').'" data-original-title="'.$this->session->userdata('employer_name').'"> <img src="'.$sender_pic.'" alt="'.$this->session->userdata('employer_name').'" height="40" width="40" /> </a> </div> <div class="chat-body"> <div class="chat-content"> <p><span class="span-msg">'.$PostArray['message'].'</span><span class="span-msg-time">'.$msg_time.'</span></p> </div> </div> </div>';
