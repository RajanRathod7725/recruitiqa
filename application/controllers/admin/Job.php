...
class Job Extends CI_Controller {
    ...
    public function __construct()
	{
        ...
        $this->thisModuleBaseUrl = site_url('admin/job').'/';
    ...
    function index($offset=0)
    {
        ...
        if($this->input->get('status') != ''){
            ...
            $where_array=array_merge($where_array,array('job.job_status'=>$suffix_array['status']));
        }
        if($this->input->get('company') != ''){
            $suffix_array['company'] = urldecode($this->input->get('company'));
            $where_array=array_merge($where_array,array('employer.employer_id'=>$suffix_array['company']));
        }


        $data['list_records']=array();
        $select_value= 'job.*,job_type.title,employer.company_name';
        ...
        $total = $this->database_model->get_joins('job',$select_value,$joins,$where_array=array('job.status !='=>'3'),'job.job_id','DESC',$this->limit,$offset,'',$like_str,'','','','Yes');
        ...
        $data['open_counter']=$this->database_model->count_all('job',array('job.status !='=>'3','job.job_status'=>'1'));
        $data['paused_counter']=$this->database_model->count_all('job',array('job.status !='=>'3','job.job_status'=>'2'));
        $data['closed_counter']=$this->database_model->count_all('job',array('job.status !='=>'3','job.job_status'=>'3'));
        /*******************************
        ||  Common data for all page ||
         *******************************/
        $data['companies']=$this->database_model->get_all_records('employer','employer_id,company_name',array('employer.status !='=>'3','employer.status !='=>'0'),'employer.employer_id DESC','')->result();
    ...
    {
        is_role_access('job','information');
        if(!is_numeric($id))
            $this->add();
        ...
        $data['row'] = $this->database_model->get_joins('job',$select_value,$joins,array('job.job_id'=>$id,'job.status !='=>'3','job.remaining_candidate >'=>0),'job.job_id','DESC',1)->row();
    ...
    function _set_rules($id='')
    {
        ...
        $this->form_validation->set_rules('code', 'Job Code', 'trim|required');
        ...
        $this->form_validation->set_rules('job_profile_size', 'No. Of Job Profile', 'trim|required');
        $this->form_validation->set_rules('job_description', 'Job Description', 'trim|required');
        $this->form_validation->set_rules('job_requirement', 'Job Requirement', 'trim|required');
    }
