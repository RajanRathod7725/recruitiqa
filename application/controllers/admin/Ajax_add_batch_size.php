...
class Ajax_add_batch_size extends CI_Controller {
    ...
    public function index()
    {
        ...
        if($csrf_check==true)
        {
            ...
            $job = $this->database_model->get_all_records('job','employer_id',array('job_id'=>$PostArray['id']),'job_id ASC',1,'')->row();

            $subscription = $this->common_model->check_subscription($job->employer_id,$PostArray['size']);
