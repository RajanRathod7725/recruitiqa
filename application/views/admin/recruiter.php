...
<div class="app-content content">
    ...
    <div class="content-wrapper">
        ...
        <div class="content-header row">
            ...
            <?php if(is_role_access('recruiter','add','display')){ ?>
                                                                ...
                                                                <table class="table zero-configuration table-hover-animation">
                                                                    ...
                                                                    else
                                                                    {
                                                                        ...
                                                                        foreach($list_records as $row){  ?>
                                                                            <tr class="data <?php echo $i%2==0 ? "alter" : ""; ?>" id="data-<?php echo $row->recruiter_id; ?>">
                                                                                <td>
                                                                                    <div class="avatar mr-1 avatar-lg">
                                                                                    <img src="<?php echo check_image($row->recruiter_photo,'uploads/recruiter','thumb'); ?>" class="img-responsive thumb-sm" height="45" width="45"/>
                                                                                    </div>
                                                                                </td>
                                                                                <td><?php echo $this->common_model->filterOutput($row->recruiter_name); ?></td>
                                                                                <td><?php echo $this->common_model->filterOutput($row->recruiter_email); ?></td>
                                                                                <td><?php echo $this->common_model->filterOutput($row->recruiter_linkedin); ?></td>
                                                                                <td class="action">
                                                                            <?php if(is_role_access('recruiter','status','display')){ ?>
                                                                                    <?php if($row->status==0){ echo "Not Verified!";

                                                                                    }else{?>
                                                                                    <div class="custom-control custom-switch switch-lg custom-switch-success mr-1" id="">
                                                                                    <input type="checkbox" class="list-switch custom-control-input status-switch" id="switch<?php echo $row->recruiter_id; ?>" <?php echo $row->status=='1'?'checked':'';?> >
                                                                                    <label class="custom-control-label" for="switch<?php echo $row->recruiter_id; ?>">
                                                                                ...
                                                                                <td class="action">
                                                                            <?php if(is_role_access('recruiter','edit','display')){ ?>
                                                                                    <a href="<?php echo $edit_link.$row->recruiter_id; ?>" class="" title="Edit"><i class="feather icon-edit-2"></i></a>
                                                                                <?php }if(is_role_access('recruiter','delete','display')){ ?>
                                                                                    <a href="javascript:;" class="delete" title="Delete" ><i class="feather icon-trash"></i></a>
                                                                                <?php } ?>
                                                                                <a data-toggle="tooltip" data-original-title="Login as Recruiter" href="<?php echo $login_recruiter.$row->recruiter_id; ?>"><i class="feather icon-user"></i></a>
                                                                                </td>
                                                                            </tr>
                                                                        <?php }
                                                                    } ?>
